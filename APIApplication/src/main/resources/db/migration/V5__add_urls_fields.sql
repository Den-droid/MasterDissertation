...
VALUES ('/api/assignments', 'Отримати список завдань користувача', 0),
       ...
       ('/api/assignments/{userAssignmentId}/start', 'Розпочати або продовжити виконання завдання', 2),
...
VALUES ('userId', 'Ідентифікатор', 'Унікальний числовий ідентифікатор користувача', 0),
       ...
       ('answer', 'Відповідь', 'Текст відповіді користувача: якщо завдання - функція:' ||
                               'x1=0;x2=-4.3;... якщо завдання лабіринт, то: 0 або left (вліво), 1 або up (вверх), ' ||
                               '2 або right (вправо), 3 або down (вниз)', 2),
       ('mark', 'Оцінка', 'Отримана оцінка за завдання', 0),
       ('restrictionType', 'Тип обмеження', 'Тип обмеження для виконання завдання', 4),
       ('attemptsRemaining', 'Кількість спроб', 'Кількість доступних спроб', 0),
       ('deadline', 'Дедлайн', 'Кінцевий термін виконання завдання', 3),
       ('minutesToDo', 'Кількість хвилин', 'Хвилин для виконання завдання', 0),
       ...
       ('text', 'Текст функції', 'Текст функції у форматі: ' ||
                                 'x1^2+x2^3...', 2),
       ('variablesCount', 'Кількість змінних', 'Кількість змінних у функції', 0),
...
values ((select id from urls where url = '/api/assignments/assignFunctionToGroup'),
       ...
       ((select id
         ...
         where url = '/api/assignmentRestrictions/setDefaultRestriction'
           and method = 2),
        (select id from fields where name = 'minutesToDo'),
        false, false),
       ((select id
         from urls
         where url = '/api/assignmentRestrictions/setDefaultRestriction'
